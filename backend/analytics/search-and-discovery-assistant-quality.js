/**
 * Analytics event instrumentation: search-and-discovery-assistant-quality
 * Task: Instrument analytics for search and discovery assistant quality: helpfulness vot
 */
'use strict';

const EVENT_PREFIX = 'SEARCH_AND_DISCOVERY_ASSISTANT_QUALITY';
...
/**
 * Normalize event payload to ensure consistent shape across all search-and-discovery-assistant-quality events.
...
function normalizePayload(type, data = {}) {
  return {
    ...
    properties: {
      ...data,
      source: 'search-and-discovery-assistant-quality',
    },
  };
}
...
function dispatchEvent(payload) {
  ...
  if (typeof process !== 'undefined' && process.env.NODE_ENV !== 'test') {
    console.info('[analytics:search-and-discovery-assistant-quality]', JSON.stringify(payload));
  }
}
