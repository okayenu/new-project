/**
 * Analytics event instrumentation: checkout-orchestration-assistant-quality
 * Task: Instrument analytics for checkout orchestration assistant quality: helpfulness v
 */
'use strict';

const EVENT_PREFIX = 'CHECKOUT_ORCHESTRATION_ASSISTANT_QUALITY';
...
/**
 * Normalize event payload to ensure consistent shape across all checkout-orchestration-assistant-quality events.
...
function normalizePayload(type, data = {}) {
  return {
    ...
    properties: {
      ...data,
      source: 'checkout-orchestration-assistant-quality',
    },
  };
}
...
function dispatchEvent(payload) {
  ...
  if (typeof process !== 'undefined' && process.env.NODE_ENV !== 'test') {
    console.info('[analytics:checkout-orchestration-assistant-quality]', JSON.stringify(payload));
  }
}
