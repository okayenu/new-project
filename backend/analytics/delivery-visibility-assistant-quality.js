/**
 * Analytics event instrumentation: delivery-visibility-assistant-quality
 * Task: Instrument analytics for delivery visibility assistant quality: helpfulness vote
 */
'use strict';

const EVENT_PREFIX = 'DELIVERY_VISIBILITY_ASSISTANT_QUALITY';
...
/**
 * Normalize event payload to ensure consistent shape across all delivery-visibility-assistant-quality events.
...
function normalizePayload(type, data = {}) {
  return {
    ...
    properties: {
      ...data,
      source: 'delivery-visibility-assistant-quality',
    },
  };
}
...
function dispatchEvent(payload) {
  ...
  if (typeof process !== 'undefined' && process.env.NODE_ENV !== 'test') {
    console.info('[analytics:delivery-visibility-assistant-quality]', JSON.stringify(payload));
  }
}
